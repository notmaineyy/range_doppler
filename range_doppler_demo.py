...
Algorithm (RDA) and shows how five physical variables trade off between the
...
* Range axis  : fast-time (echo delay).  R = c*tau/2.   Clarity limited by
                the range resolution  dR = c / (2*B),  so it is set by the
                transmitted BANDWIDTH B.  B does NOT move the target.
* Doppler axis: slow-time (platform motion).  The target traces a quadratic
                ...
                (cross-range) resolution  rho_az = La/2.  Neither La nor vp
                changes a stationary target's azimuth position.

Variables exercised (each produces one figure in ./outputs)
-----------------------------------------------------------
1. Bandwidth B          -> range clarity (dR = c/2B), position unchanged.
2. Radial velocity vr   -> Doppler-centroid shift fd = 2*vr/lam, which
                          displaces the target in azimuth by dx = vr*R0/vp
                          and causes range walk  vr*Ta  (position error).
3. Antenna length La    -> azimuth clarity (rho_az = La/2, Bd = 2*vp/La),
                          position unchanged.
...
def main():
    ...
    print(f"  dR          = {d['dR']:.2f} m  (range resolution)")
    print(f"  rho_az      = {d['rho_az']:.2f} m  (azimuth resolution = La/2)")
