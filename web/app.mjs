...
for(const key of ['dr','da','shift','walk']){const value=key==='dr'?s.rangeResolution:key==='da'?s.azimuthResolution:s[key];document.querySelector('#'+key).textContent=(key==='shift'&&value>=0?'+':'')+value.toFixed(2)+' m';}
 document.querySelector('#resolution-values').textContent=`βr = ${s.rangeBroadening.toFixed(3)} (no range taper); βa = ${s.azimuthBroadening.toFixed(3)} (${s.params.window==='Rectangular'?'None':s.params.window}). Processed synthetic aperture Lsyn = ${s.syntheticLength.toFixed(2)} m; effective along-track aperture Leff = Lsyn cos²θ = ${s.effectiveAperture.toFixed(2)} m. Broadside formula λRβa/(2Lsyn) = ${s.azimuthBroadsideResolution.toFixed(3)} m; current along-track resolution λRβa/(2Leff) = ${s.azimuthResolution.toFixed(3)} m. Predicted range half-power width ≈ ${(0.885892941*s.rangeResolution).toFixed(3)} m.`;
document.querySelector('#width').textContent=`Measured azimuth −3 dB width: ${s.width===null?'not measurable':s.width.toFixed(2)+' m'} · Weighting broadening β = ${s.broadening.toFixed(3)}× · Stationary prediction: ${s.nominalWidth.toFixed(2)} m`;
