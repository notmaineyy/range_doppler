...
function updateMetrics(){const s=result;for(const key of ['dr','da','shift','walk'])document.querySelector('#'+key).textContent=(key==='shift'&&s[key]>=0?'+':'')+s[key].toFixed(2)+' m';document.querySelector('#width').textContent=`Measured azimuth −3 dB width: ${s.width.toFixed(2)} m · ${s.params.window} weighting`;
 document.querySelector('#peak').textContent=`Brightest pixel: range ${(5000+s.peakR).toFixed(2)} m, azimuth ${s.peakX.toFixed(2)} m. True centre-time position: (5000 m, 0 m).`+(Math.abs(s.shift)>Math.max(8,4*s.da)&&!s.params.full?' Enable full position range to see the true marker.':'');
...
function heat(id,z,cols,rows,ymin,ymax,ylabel){const o=setup(id),{c,left,top,pw,ph}=o;
 ...
 bc.putImageData(pixels,0,0);c.imageSmoothingEnabled=true;c.drawImage(buffer,left,top,pw,ph);axes(o,-15,15,ymin,ymax,'Slant-range offset from 5 km [m]',ylabel);
 ...
 const yPos=y=>top+ph*(ymax-y)/(ymax-ymin);
 c.save();c.beginPath();c.rect(left,top,pw,ph);c.clip();c.strokeStyle='#52ebed';c.lineWidth=1.5;
 if(id==='rd'){c.setLineDash([5,4]);c.beginPath();c.moveTo(left,yPos(result.fd));c.lineTo(left+pw,yPos(result.fd));c.stroke();}
 else{const x=left+pw/2,y=yPos(0),yp=yPos(result.shift);c.beginPath();c.moveTo(x-7,y);c.lineTo(x+7,y);c.moveTo(x,y-7);c.lineTo(x,y+7);c.stroke();c.strokeStyle='#a9f76a';c.beginPath();c.arc(x,yp,8,0,2*Math.PI);c.stroke();}
 c.restore();}
...
function paint(){if(paintPending)return;paintPending=true;requestAnimationFrame(()=>{paintPending=false;if(!result)return;const s=result;heat('rd',s.rd,s.dcols,s.drows,-800,800,'Doppler frequency [Hz]');heat('image',s.image,s.cols,s.rows,s.xAxis[0],s.xAxis[s.rows-1],'Azimuth [m]');line('rangeProfile',s.rAxis,s.rp,-15,15,'Slant-range offset [m]');line('azimuthProfile',s.xAxis,s.ap,s.shift-10,s.shift+10,'Azimuth [m]');});}
